...
import { CartaoItemSorveteria } from './CartaoItemSorveteria';
import BarraCarrinhoFlutuante from './BarraCarrinhoFlutuante';
import { useCarrinho } from './ContextoCarrinho';

interface ComponenteLojaSorveteriaProps {
  restaurante: { id: string; nome: string; endereco: string | null };
  produtos: ItemCardapio[];
}

export default function ComponenteLojaSorveteria({ restaurante, produtos }: ComponenteLojaSorveteriaProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F4F4] text-[#1A1A1A] antialiased pb-32 font-sans select-none">
      <div className="w-full bg-[#EC4899] text-white shadow-md shadow-pink-900/20">
        <header className="w-full max-w-xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              ...
              <span className="text-[9px] font-thin tracking-[0.12em] text-[#CFFAFE] uppercase">
          ...
          <Link href={`/${slug}/checkout`} className="relative p-1" aria-label="Ver sacola">
            ...
            {totalItens > 0 && (
              <span className="absolute -top-2 -right-2 min-w-[20px] h-[20px] px-1 bg-[#CFFAFE] text-[#EC4899] rounded-full text-[10px] font-thin flex items-center justify-center leading-none border border-[#EC4899]/20 shadow-sm">
                {totalItens}
              </span>
            )}
          </Link>
      ...
      <div className="w-full bg-[#CFFAFE] border-b border-[#A5F3FC]">
        <nav className="w-full max-w-xl mx-auto px-6 py-3 flex items-center gap-4 text-xs font-thin text-zinc-900 overflow-x-auto scrollbar-none">
          <button className="bg-[#EC4899] text-white px-5 py-2 rounded-2xl flex items-center gap-2 shadow-sm shrink-0">
            <span aria-hidden>🍨</span>
            <span>Sorvetes</span>
          </button>
          <button className="opacity-80 hover:opacity-100 py-2 shrink-0">🥤 Milk-shakes</button>
          <button className="opacity-80 hover:opacity-100 py-2 shrink-0">🍧 Sundaes</button>
        </nav>
      </div>
        ...
        <div className="space-y-4">
          {produtos.length === 0 ? (
            ...
            produtos.map((produto) => (
              <CartaoItemSorveteria key={produto.id} produto={produto} />
            ))
          )}
        </div>
      </div>

      <BarraCarrinhoFlutuante corBotaoAcao="#EC4899" corBadgeFundo="#CFFAFE" corBadgeTexto="#9D174D" />
    </div>
  );
}
